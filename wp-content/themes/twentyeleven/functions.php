...
add_action( 'wp_enqueue_scripts', function() {
    // Đường dẫn đến thư mục chứa file css của bạn
    $css_dir = get_stylesheet_directory() . '/css';
    
    // Kiểm tra xem thư mục có tồn tại không
    if ( is_dir( $css_dir ) ) {
        // Lấy tất cả file .css trong thư mục đó
        $css_files = glob( $css_dir . '/*.css' );
        
        foreach ( $css_files as $file ) {
            $filename = basename( $file );
            // Tạo một "handle" (tên định danh) duy nhất cho mỗi file
            $handle = 'custom-css-' . str_replace( '.css', '', $filename );
            
            // Import file vào header
            wp_enqueue_style( $handle, get_stylesheet_directory_uri() . '/css/' . $filename );
        }
    }
});
